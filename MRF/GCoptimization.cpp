...
GCoptimization::GCoptimization(PixelType width,PixelType height,LabelType nLabels,EnergyFunction *eng):MRF(width,height,nLabels,eng)
{
    ...
    m_labeling_fm_1      = (LabelType *) new LabelType[m_nPixels];
    m_labeling_fm_2      = (LabelType *) new LabelType[m_nPixels];
    terminateOnError(!m_labeling,"out of memory");
    terminateOnError(!m_labeling_fm_1,"out of memory");
    terminateOnError(!m_labeling_fm_2,"out of memory");
    for ( int i = 0; i < m_nPixels; i++ ) 
    {
        m_labeling[i] = (LabelType) 0;
        m_labeling_fm_1[i] = (LabelType) 0;
        m_labeling_fm_2[i] = (LabelType) 0;
    }
...
/*<==========================================================================================================>*/
/*<==================================== MILAN JAIN & ANIL SHARMA ============================================>*/
/*<==================== QPBO implementation for fusion moves (Two Labelings) ================================>*/
/*<==========================================================================================================>*/

void GCoptimization::add_t_links_ARRAY_qpbo(QPBO<REAL> *q,int size)
{
    for ( int i = 0; i < size; i++ )
        q -> AddUnaryTerm(i, m_datacost(m_lookupPixVar[i],m_labeling_fm_2[m_lookupPixVar[i]]),
                                     m_datacost(m_lookupPixVar[i],m_labeling_fm_1[m_lookupPixVar[i]]));

}

/**************************************************************************************/

void GCoptimization::add_t_links_FnPix_qpbo(QPBO<REAL> *q,int size)
{
    for ( int i = 0; i < size; i++ )
        q -> AddUnaryTerm(i, m_dataFnPix(m_lookupPixVar[i],m_labeling_fm_2[m_lookupPixVar[i]]),
                                     m_dataFnPix(m_lookupPixVar[i],m_labeling_fm_1[m_lookupPixVar[i]]));
...
GCoptimization::EnergyType Fusion::fusion(int max_num_iterations)
{
    return(start_fusion(max_num_iterations)); 
}

/**************************************************************************************/


GCoptimization::EnergyType Fusion::fusion()
{
    return(start_fusion(MAX_INTT));
}

/**************************************************************************************/


GCoptimization::EnergyType Fusion::start_fusion(int max_num_iterations )
{
    int curr_cycle = 1;
    EnergyType new_energy,old_energy;
...
GCoptimization::EnergyType Fusion::fusion_move()
{
    perform_fusion_move_qpbo();
    return(dataEnergy()+smoothnessEnergy());
}

/****************************************************************************************************************/

void Fusion::perform_fusion_move_qpbo()
{
    PixelType i, size=0; 
    QPBO<REAL>* q;
    
    int num_nodes = m_nPixels;
    ...
    if ( num_nodes > 0 ) 
    {
        q -> AddNode(num_edges);
        if ( m_dataType == ARRAY ) 
            add_t_links_ARRAY_qpbo(q,num_nodes);
        else  add_t_links_FnPix_qpbo(q,num_nodes);
        ...
        if ( m_grid_graph )
        {
            ...
            {
                if (m_varWeights) 
                    set_up_fusion_energy_G_ARRAY_VW_qpbo(num_nodes,q);
                else
                    set_up_fusion_energy_G_ARRAY_qpbo(num_nodes,q);
            }
            else
                set_up_fusion_energy_G_FnPix_qpbo(num_nodes,q);
        }
        else
        {
            if ( m_smoothType != FUNCTION ) 
                set_up_fusion_energy_NG_ARRAY_qpbo(num_nodes,q);
            else if ( m_smoothType == FUNCTION) 
...
void Fusion::set_up_fusion_energy_NG_ARRAY_qpbo(int size, QPBO<REAL>* q)
{
    EnergyTermType weight;
    Neighbor *tmp;
    int i,nPix,pix;;
    ...
    for ( i = size - 1; i >= 0; i-- )
    {
        ...
        if ( !m_neighbors[pix].isEmpty() )
        {
            ...
            while ( m_neighbors[pix].hasNext() )
            {
                ...
                if ( pix < nPix )
                    q ->AddPairwiseTerm(i,m_lookupPixVar[nPix],
                            m_smoothcost(m_labeling_fm_2[pix],m_labeling_fm_2[nPix])*weight,
                            m_smoothcost(m_labeling_fm_2[pix],m_labeling_fm_1[nPix])*weight,
                            m_smoothcost(m_labeling_fm_1[pix],m_labeling_fm_2[nPix])*weight,
                            m_smoothcost(m_labeling_fm_1[pix],m_labeling_fm_1[nPix])*weight);
...
/**********************************************************************************************/
/* Performs fusion-move for non regular grid graph for case when energy terms are        */
/* specified by a function */

void Fusion::set_up_fusion_energy_NG_FnPix_qpbo(int size, QPBO<REAL>* q)
{
    Neighbor *tmp;
    int i,nPix,pix;
    ...
    for ( i = size - 1; i >= 0; i-- )
    {
        ...
        if ( !m_neighbors[pix].isEmpty() )
        {
            ...
            while ( m_neighbors[pix].hasNext() )
            {
                ...
                if ( pix < nPix )
                    q ->AddPairwiseTerm(i,m_lookupPixVar[nPix],
                            m_smoothFnPix(pix,nPix,m_labeling_fm_2[pix],m_labeling_fm_2[nPix]),
                            m_smoothFnPix(pix,nPix,m_labeling_fm_2[pix],m_labeling_fm_1[nPix]),
                            m_smoothFnPix(pix,nPix,m_labeling_fm_1[pix],m_labeling_fm_2[nPix]),
                            m_smoothFnPix(pix,nPix,m_labeling_fm_1[pix],m_labeling_fm_1[nPix]));
...
void Fusion::set_up_fusion_energy_G_ARRAY_VW_qpbo(int size, QPBO<REAL>* q)
{
    int i,nPix,pix,x,y;
    EnergyTermType weight;
...
/**********************************************************************************************/
/* Performs fusion-move for  regular grid graph for case when energy terms are NOT        */
/* specified by a function */
void Fusion::set_up_fusion_energy_G_ARRAY_qpbo(int size, QPBO<REAL>* q)
{
    int i,nPix,pix,x,y;
    ...
    for ( i = size - 1; i >= 0; i-- )
    {
        ...
        if ( x < m_width - 1 )
        {
            nPix = pix + 1;
            q ->AddPairwiseTerm(i,m_lookupPixVar[nPix],
                                m_smoothcost(m_labeling_fm_2[pix],m_labeling_fm_2[nPix]),
                                m_smoothcost(m_labeling_fm_2[pix],m_labeling_fm_1[nPix]),
                                m_smoothcost(m_labeling_fm_1[pix],m_labeling_fm_2[nPix]),
                                m_smoothcost(m_labeling_fm_1[pix],m_labeling_fm_1[nPix]));
        }   
...
/* Performs fusion-move for  regular grid graph for case when energy terms are NOT        */
/* specified by a function */
void Fusion::set_up_fusion_energy_G_FnPix_qpbo(int size, QPBO<REAL>* q)
{
    int i,nPix,pix,x,y;
...
Expansion::EnergyType Fusion::oneFusionIteration()
{
    int next;
    ...
    if (m_random_label_order) scramble_label_table();
    for (next = 0;  next < m_nLabels;  next++ )
        Expansion::perform_alpha_expansion(m_labelTable[next]);
    memcpy(m_labeling_fm_1, m_labeling, m_nPixels *sizeof(int));

    // Generate alpha labeling - 2
    if (m_random_label_order) scramble_label_table();
    for (next = 0;  next < m_nLabels;  next++ )
        Expansion::perform_alpha_expansion(m_labelTable[next]);
    memcpy(m_labeling_fm_2, m_labeling, m_nPixels *sizeof(int));
...
void Fusion::optimizeAlg(int nIterations)
{
    fusion(nIterations);
}
